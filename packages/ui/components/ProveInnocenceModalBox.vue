<template>
  <div class="modal-card box box-modal">
    ...
    <b-button type="is-primary is-fullwidth" data-test="withdrawal_confirm_button" @click="_sendProof">
      {{ $t('confirm') }}
    </b-button>
  </div>
</template>
  ...
  computed: {
    withdrawalMethod() {
      if (this.withdrawType === 'wallet') {
        return 'application/proveInnocence'
      } else {
        throw new Error('Cannot prove innocence only via a relayer')
      }
    }
  },
  methods: {
